...
static int commandMapGetReverse(CommandMap* commandMap, jint value);
static int javaCommandToBrltty(jint javaCommand, jint javaArgumentMode,
                               jint javaArgumentValue, int *outCommand,
                               int *outArgumentMode, int *outArgumentValue);
...
static jclass class_BrailleInputEvent;
...
static jmethodID method_BrailleInputEvent_ctor;
...
jobject
Java_com_googlecode_eyesfree_braille_service_display_BrlttyWrapper_readInputEventNative(
    JNIEnv* env, jobject thiz) {
  int readDelayMillis = -1;
  while (1) {
    int innerDelayMillis = -1;
    brltty_pumpCommands(&innerDelayMillis);
    if (readDelayMillis < 0 ||
        (innerDelayMillis > 0 && innerDelayMillis < readDelayMillis)) {
      readDelayMillis = innerDelayMillis;
    }
    ...
    if (brltty_popInputEvent(&event) == EOF) {
      if (readDelayMillis > 0) {
        (*env)->CallVoidMethod(env, thiz, method_readDelayed,
                               (jlong) readDelayMillis);
      }
    ...
    jint mappedCommand = -1;
    jint mappedArg = 0;
    if (event.type == BRLTTY_INPUT_EVENT_COMMAND) {
      mapBrlttyCommand(event.command, &mappedCommand, &mappedArg);
      if (mappedCommand < 0) {
        continue;
      }
    } else {
    ...
    jstring bindingSignature = NULL;
    if (event.bindingSignature[0]) {
      bindingSignature = (*env)->NewStringUTF(env, event.bindingSignature);
      if (bindingSignature == NULL) {
        return NULL;
      }
    }

    jobject inputEvent = (*env)->NewObject(
        env, class_BrailleInputEvent, method_BrailleInputEvent_ctor,
        mappedCommand, mappedArg, (jlong) 0,
        event.type,
        event.rawGroup, event.rawNumber, event.rawPress ? JNI_TRUE : JNI_FALSE,
        bindingSignature, event.rawRemapped ? JNI_TRUE : JNI_FALSE);
    if (bindingSignature != NULL) {
      (*env)->DeleteLocalRef(env, bindingSignature);
    }
    if (readDelayMillis > 0) {
      (*env)->CallVoidMethod(env, thiz, method_readDelayed,
                             (jlong) readDelayMillis);
...
void
Java_com_googlecode_eyesfree_braille_service_display_BrlttyWrapper_configureRawRemapNative(
    JNIEnv* env, jobject thiz, jobjectArray signatures, jintArray targetCommands,
    jintArray targetArgumentModes, jintArray targetArgumentValues) {
  if (signatures == NULL || targetCommands == NULL
      || targetArgumentModes == NULL || targetArgumentValues == NULL) {
    brltty_configureRawRemaps(0, NULL, NULL, NULL, NULL);
    return;
  }

  jsize signatureCount = (*env)->GetArrayLength(env, signatures);
  jsize commandCount = (*env)->GetArrayLength(env, targetCommands);
  jsize modeCount = (*env)->GetArrayLength(env, targetArgumentModes);
  jsize valueCount = (*env)->GetArrayLength(env, targetArgumentValues);
  jsize count = signatureCount;
  if (commandCount < count) count = commandCount;
  if (modeCount < count) count = modeCount;
  ...
  const char **nativeSignatures = calloc(count, sizeof(*nativeSignatures));
  int *nativeCommands = calloc(count, sizeof(*nativeCommands));
  int *nativeArgumentModes = calloc(count, sizeof(*nativeArgumentModes));
  int *nativeArgumentValues = calloc(count, sizeof(*nativeArgumentValues));
  if (!nativeSignatures || !nativeCommands || !nativeArgumentModes
      || !nativeArgumentValues) {
    free(nativeSignatures);
    free(nativeCommands);
    free(nativeArgumentModes);
    free(nativeArgumentValues);
    (*env)->ThrowNew(env, class_OutOfMemoryError, NULL);
    return;
  }

  jint *javaTargets = (*env)->GetIntArrayElements(env, targetCommands, NULL);
  jint *javaArgumentModes = (*env)->GetIntArrayElements(env, targetArgumentModes,
      NULL);
  jint *javaArgumentValues = (*env)->GetIntArrayElements(env,
      targetArgumentValues, NULL);
  if (!javaTargets || !javaArgumentModes || !javaArgumentValues) {
    free(nativeSignatures);
    free(nativeCommands);
    free(nativeArgumentModes);
    free(nativeArgumentValues);
    if (javaTargets) {
      (*env)->ReleaseIntArrayElements(env, targetCommands, javaTargets,
                                      JNI_ABORT);
    }
    if (javaArgumentModes) {
      (*env)->ReleaseIntArrayElements(env, targetArgumentModes,
                                      javaArgumentModes, JNI_ABORT);
    }
    if (javaArgumentValues) {
      (*env)->ReleaseIntArrayElements(env, targetArgumentValues,
                                      javaArgumentValues, JNI_ABORT);
    }
    return;
  }
  ...
  for (jsize i = 0; i < count; ++i) {
    jstring signature = (jstring) (*env)->GetObjectArrayElement(env, signatures, i);
    if (signature == NULL) {
      continue;
    }
    const char *signatureChars = (*env)->GetStringUTFChars(env, signature, NULL);
    if (!signatureChars) {
      (*env)->DeleteLocalRef(env, signature);
      continue;
    }
    int brlttyCommand;
    int brlttyArgumentMode;
    int brlttyArgumentValue;
    if (javaCommandToBrltty(javaTargets[i], javaArgumentModes[i],
            javaArgumentValues[i], &brlttyCommand, &brlttyArgumentMode,
            &brlttyArgumentValue)) {
      nativeSignatures[actualCount] = strdup(signatureChars);
      if (nativeSignatures[actualCount]) {
        nativeCommands[actualCount] = brlttyCommand;
        nativeArgumentModes[actualCount] = brlttyArgumentMode;
        nativeArgumentValues[actualCount] = brlttyArgumentValue;
        actualCount += 1;
      }
    }
    (*env)->ReleaseStringUTFChars(env, signature, signatureChars);
    (*env)->DeleteLocalRef(env, signature);
  }

  (*env)->ReleaseIntArrayElements(env, targetCommands, javaTargets, JNI_ABORT);
  (*env)->ReleaseIntArrayElements(env, targetArgumentModes, javaArgumentModes,
                                  JNI_ABORT);
  (*env)->ReleaseIntArrayElements(env, targetArgumentValues,
                                  javaArgumentValues, JNI_ABORT);
  brltty_configureRawRemaps(actualCount, nativeSignatures, nativeCommands,
                            nativeArgumentModes, nativeArgumentValues);

  for (int i = 0; i < actualCount; ++i) {
    free((void *)nativeSignatures[i]);
  }
  free(nativeSignatures);
  free(nativeCommands);
  free(nativeArgumentModes);
...
Java_com_googlecode_eyesfree_braille_service_display_BrlttyWrapper_classInitNative(
  ...
  if (!(class_BrailleInputEvent = getGlobalClassRef(
          env, DISPLAY_PACKAGE "BrailleInputEvent"))) {
    return;
  }
  if (!(method_BrailleInputEvent_ctor =
        (*env)->GetMethodID(
            env, class_BrailleInputEvent, "<init>",
            "(IIJIIIZLjava/lang/String;Z)V"))) {
    return;
  }
...
static int
commandMapGetReverse(CommandMap* commandMap, jint value) {
  if (!commandMap) {
    return -1;
  }
  for (size_t i = 0; i < commandMap->numEntries; ++i) {
    if (commandMap->entries[i].javaValue == value) {
      return commandMap->entries[i].brlttyValue;
    }
  }
  return -1;
}
...
static int
javaCommandToBrltty(jint javaCommand, jint javaArgumentMode,
                    jint javaArgumentValue, int *outCommand,
                    int *outArgumentMode, int *outArgumentValue) {
  int command = commandMapGetReverse(brlttyCommandMap, javaCommand);
  int argumentMode = javaArgumentMode;
  int argumentValue = javaArgumentValue;

  if (command < 0) {
    int brlttyKey = commandMapGetReverse(brlttyKeyMap, javaCommand);
    if (brlttyKey >= 0) {
      command = BRL_CMD_ARG(PASSKEY, brlttyKey);
      argumentMode = BRLTTY_REMAP_ARG_NONE;
      argumentValue = 0;
    }
  }

  if (javaCommand == cmdRoute) {
    command = BRL_CMD_BLK(ROUTE);
  } else if (javaCommand == cmdLongPressRoute) {
    command = BRL_CMD_BLK(ROUTE) | BRLTTY_ROUTE_ARG_FLG_LONG_PRESS;
  } else if (javaCommand == cmdActivateCurrent) {
    command = BRL_CMD_BLK(ROUTE);
    argumentMode = BRLTTY_REMAP_ARG_FIXED;
    argumentValue = BRLTTY_MAX_TEXT_CELLS;
  } else if (javaCommand == cmdLongPressCurrent) {
    command = BRL_CMD_BLK(ROUTE) | BRLTTY_ROUTE_ARG_FLG_LONG_PRESS;
    argumentMode = BRLTTY_REMAP_ARG_FIXED;
    argumentValue = BRLTTY_MAX_TEXT_CELLS;
  }
  ...
  if (outArgumentMode) {
    *outArgumentMode = argumentMode;
  }
  if (outArgumentValue) {
    *outArgumentValue = argumentValue;
  }
  return 1;
}
