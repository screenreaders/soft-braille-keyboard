...
resetLongPressData (KeyTable *table) {
  ...
  table->longPress.bindingSignature[0] = 0;
}
...
compileKeyTable (const char *name, KEY_NAME_TABLES_REFERENCE keys) {
  ...
  if (setTableDataVariables(KEY_TABLE_EXTENSION, KEY_SUBTABLE_EXTENSION)) {
    ...
    if ((ktd.table = malloc(sizeof(*ktd.table)))) {
      ...
      ktd.table->rawRewrite.heldKeys = NULL;
      ktd.table->rawRewrite.heldSize = 0;
      ktd.table->rawRewrite.heldCount = 0;
      ktd.table->rawRewrite.heldPresses = NULL;
      ktd.table->rawRewrite.heldPressesSize = 0;
      ktd.table->rawRewrite.heldPressesCount = 0;
      ktd.table->rawRewrite.holdActive = 0;
      ktd.table->rawRewrite.suppressReleases = 0;
...
destroyKeyTable (KeyTable *table) {
  ...
  if (table->rawRewrite.heldKeys) free(table->rawRewrite.heldKeys);
  if (table->rawRewrite.heldPresses) free(table->rawRewrite.heldPresses);
  free(table);
}
