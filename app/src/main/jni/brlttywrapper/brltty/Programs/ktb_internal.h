...
#define MAX_BINDING_SIGNATURE_LENGTH 0X100
...
struct KeyTableStruct {
  ...
  struct {
    KeyValue *heldKeys;
    unsigned int heldSize;
    unsigned int heldCount;

    KeyValue *heldPresses;
    unsigned int heldPressesSize;
    unsigned int heldPressesCount;

    unsigned holdActive:1;
    unsigned suppressReleases:1;
  } rawRewrite;

  struct {
    int command;
    char bindingSignature[MAX_BINDING_SIGNATURE_LENGTH];
    unsigned rawRemapped:1;
  } release;
