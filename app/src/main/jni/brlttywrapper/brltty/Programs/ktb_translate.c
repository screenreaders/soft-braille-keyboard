...
static const char *
findBindingKeyName(const KeyTable *table, const KeyValue *value) {
  const KeyNameEntry **entries = table->keyNames.table;
  int entryCount = table->keyNames.count;
  const KeyNameEntry **entry = bsearch(value, entries, entryCount,
      sizeof(KeyNameEntry*), compareValueToKeyNameEntry);
  return entry ? (*entry)->name : NULL;
}

static int
compareBindingNames(const void *a, const void *b) {
  const char *const *name1 = a;
  const char *const *name2 = b;
  return strcmp(*name1, *name2);
}

static void
buildBindingSignature(const KeyTable *table, const KeyBinding *binding,
                      int isLongPress, char *buffer, size_t size) {
  if (!buffer || size == 0) {
    return;
  }
  buffer[0] = 0;
  ...
  const char *names[MAX_MODIFIERS_PER_COMBINATION + 1];
  int nameCount = 0;

  for (int i=0; i<combination->modifierCount; i+=1) {
    const char *name = findBindingKeyName(table, &combination->modifierKeys[i]);
    if (name) {
      names[nameCount++] = name;
    }
  }
  if (combination->flags & KCF_IMMEDIATE_KEY) {
    const char *name = findBindingKeyName(table, &combination->immediateKey);
    if (name) {
      names[nameCount++] = name;
    }
  }

  qsort(names, nameCount, sizeof(*names), compareBindingNames);

  STR_BEGIN(buffer, size);
  STR_PRINTF("%s|", isLongPress ? "long" : "tap");
  for (int i=0; i<nameCount; i+=1) {
    if (i) {
      STR_PRINTF("+");
    }
    STR_PRINTF("%s", names[i]);
  }
  STR_END;
}

static void
buildSignatureFromKeyValues(const KeyTable *table, const KeyValue *keyValues,
                            unsigned int keyCount, int isLongPress,
                            char *buffer, size_t size) {
  if (!buffer || size == 0) {
    return;
  }
  buffer[0] = 0;
  if (!table || !keyValues || keyCount == 0) {
    return;
  }

  const char *names[MAX_MODIFIERS_PER_COMBINATION + 1];
  unsigned int nameCount = 0;
  for (unsigned int i=0; i<keyCount; i+=1) {
    const char *name = findBindingKeyName(table, &keyValues[i]);
    if (name) {
      names[nameCount++] = name;
    }
  }
...
static void
applyRawRemap(const KeyTable *table, const KeyBinding *binding,
              int isLongPress, int *command,
              char *signatureBuffer, size_t signatureBufferSize,
              int *wasRemapped) {
  if (wasRemapped) {
    *wasRemapped = 0;
  }
  buildBindingSignature(table, binding, isLongPress,
                        signatureBuffer, signatureBufferSize);
  if (!signatureBuffer[0] || !command) {
    return;
  }
  brltty_applyRawRemap(signatureBuffer, command, wasRemapped);
}

static KeyTableState
processKeyEventInternal(KeyTable *table, unsigned char context,
                        KeyGroup keyGroup, KeyNumber keyNumber, int press);

static int
findHeldRawKey(KeyTable *table, const KeyValue *value, unsigned int *position) {
  return findKeyValue(table->rawRewrite.heldKeys, table->rawRewrite.heldCount,
                      value, position);
}

static int
insertHeldRawKey(KeyTable *table, const KeyValue *value, unsigned int position) {
  return insertKeyValue(&table->rawRewrite.heldKeys,
                        &table->rawRewrite.heldCount,
                        &table->rawRewrite.heldSize,
                        value, position);
}

static void
removeHeldRawKey(KeyTable *table, unsigned int position) {
  removeKeyValue(table->rawRewrite.heldKeys, &table->rawRewrite.heldCount,
                 position);
}

static int
appendHeldRawPress(KeyTable *table, const KeyValue *value) {
  return insertKeyValue(&table->rawRewrite.heldPresses,
                        &table->rawRewrite.heldPressesCount,
                        &table->rawRewrite.heldPressesSize,
                        value, table->rawRewrite.heldPressesCount);
}

static void
clearRawRewriteState(KeyTable *table) {
  table->rawRewrite.heldCount = 0;
  table->rawRewrite.heldPressesCount = 0;
  table->rawRewrite.holdActive = 0;
  table->rawRewrite.suppressReleases = 0;
}

static KeyTableState
replayHeldRawPresses(KeyTable *table, unsigned char context) {
  KeyTableState state = KTS_UNBOUND;
  unsigned int pressCount = table->rawRewrite.heldPressesCount;

  for (unsigned int i=0; i<pressCount; i+=1) {
    const KeyValue *value = &table->rawRewrite.heldPresses[i];
    state = processKeyEventInternal(table, context, value->group,
                                    value->number, 1);
  }

  clearRawRewriteState(table);
  return state;
}

static int
maybeRewriteRawChord(KeyTable *table, unsigned char context,
                     const KeyValue *keyValue, int press,
                     KeyTableState *state) {
  if (table->rawRewrite.suppressReleases) {
    if (!press) {
      unsigned int position;
      if (findHeldRawKey(table, keyValue, &position)) {
        removeHeldRawKey(table, position);
        if (table->rawRewrite.heldCount == 0) {
          clearRawRewriteState(table);
        }
        if (state) {
          *state = KTS_COMMAND;
  ...
  if (!table->rawRewrite.holdActive) {
    if (!press) {
      return 0;
    }

    unsigned int position = 0;
    findHeldRawKey(table, keyValue, &position);
    insertHeldRawKey(table, keyValue, position);
    appendHeldRawPress(table, keyValue);

    char signature[MAX_BINDING_SIGNATURE_LENGTH];
    buildSignatureFromKeyValues(table, table->rawRewrite.heldKeys,
                                table->rawRewrite.heldCount, 0,
                                signature, sizeof(signature));
    if (signature[0] && brltty_hasRawRemapPrefix(signature)) {
      table->rawRewrite.holdActive = 1;
      if (state) {
        *state = KTS_MODIFIERS;
      }
      return 1;
  ...
  if (press) {
    ...
    if (state) {
      *state = replayHeldRawPresses(table, context);
    } else {
      replayHeldRawPresses(table, context);
    }
    return 1;
  }
  ...
  int resolvedCommand = EOF;
  int wasRemapped = 0;
  if (signature[0]
      && brltty_findRawRemap(signature, &remap)
      && brltty_resolveRawRemap(&remap, 0, 0, &resolvedCommand,
                                &wasRemapped)) {
    brltty_emitCommand(resolvedCommand, signature, 1);
    removeHeldRawKey(table, position);
    table->rawRewrite.holdActive = 0;
    table->rawRewrite.heldPressesCount = 0;
    table->rawRewrite.suppressReleases =
        (table->rawRewrite.heldCount > 0);
    if (!table->rawRewrite.suppressReleases) {
      clearRawRewriteState(table);
    }
...
ASYNC_ALARM_CALLBACK(handleLongPressAlarm) {
  ...
  table->release.bindingSignature[0] = 0;
  table->release.rawRemapped = 0;
  brltty_prepareCommandMetadata(table->longPress.bindingSignature,
                                table->longPress.rawRemapped);
  ...
  if (hotkey) {
    ...
    if (press) {
      ...
      if (!wasPressed) {
        ...
        char bindingSignature[MAX_BINDING_SIGNATURE_LENGTH];
        char longPressSignature[MAX_BINDING_SIGNATURE_LENGTH];
        int rawRemapped = 0;
        int longPressRawRemapped = 0;
        bindingSignature[0] = 0;
        longPressSignature[0] = 0;
        ...
        if (binding) {
          ...
          applyRawRemap(table, binding, 0, &command,
                        bindingSignature,
                        sizeof(bindingSignature),
                        &rawRemapped);
          ...
          applyRawRemap(table, binding, 1, &secondaryCommand,
                        longPressSignature,
                        sizeof(longPressSignature),
                        &longPressRawRemapped);
        }
        ...
        if (context == KTB_CTX_WAITING) {
          ...
          if (isImmediate) {
            ...
            snprintf(table->release.bindingSignature,
                     sizeof(table->release.bindingSignature), "%s",
                     bindingSignature);
            table->release.rawRemapped = rawRemapped;
            command = BRL_CMD_NOOP;
          }
          ...
          if (secondaryCommand != BRL_CMD_NOOP) {
            ...
            table->longPress.rawRemapped = longPressRawRemapped;
            snprintf(table->longPress.bindingSignature,
                     sizeof(table->longPress.bindingSignature), "%s",
                     longPressSignature);
      ...
      if (ON_FIRST_RELEASE || (table->pressedKeys.count == 0)) {
        ...
        if (*cmd != BRL_CMD_NOOP) {
          brltty_prepareCommandMetadata(table->release.bindingSignature,
                                        table->release.rawRemapped);
...
processKeyEvent (
  ...
  if (maybeRewriteRawChord(table, context, &keyValue, press, &state)) {
    return state;
  }

  return processKeyEventInternal(table, context, keyGroup, keyNumber, press);
}
