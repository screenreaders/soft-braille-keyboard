...
public class DisplayService extends Service
        ...
        new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                if (Intent.ACTION_SCREEN_ON.equals(action)) {
                    ...
                    disconnectBraille();
                } else if (UsbHelper.ACTION_USB_PERMISSION.equals(action)) {
    ...
    public IBinder onBind(Intent intent) {
        ...
        if (hasBluetoothConnectPermission() || new DeviceFinder(this)
                .findUsbDevices().size() > 0) {
            connectBraille();
        } else {
            Log.w(LOG_TAG, "Braille service has no connectable devices yet");
        }
        return mServiceImpl;
    }
