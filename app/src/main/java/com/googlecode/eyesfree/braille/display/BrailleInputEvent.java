...
public class BrailleInputEvent implements Parcelable {
    ...
    /** Keyboard command: Toggle the Braille grade. */
    ...
    /** Event type: a translated braille command. */
    public static final int TYPE_COMMAND = 0;
    /** Event type: a raw physical key transition. */
    public static final int TYPE_RAW_KEY = 1;
    ...
    private final int mRawGroup;
    private final int mRawNumber;
    private final boolean mRawPress;
    private final String mBindingSignature;
    private final boolean mRawRemapped;

    public BrailleInputEvent(int command, int argument, long eventTime) {
        this(command, argument, eventTime, TYPE_COMMAND, -1, -1, false, null,
                false);
    }

    public BrailleInputEvent(int command, int argument, long eventTime,
            int eventType, int rawGroup, int rawNumber, boolean rawPress,
            String bindingSignature, boolean rawRemapped) {
        mCommand = command;
        mArgument = argument;
        ...
        mRawPress = rawPress;
        mBindingSignature = bindingSignature;
        mRawRemapped = rawRemapped;
    }
    ...
    public boolean isCommandEvent() {
        return mEventType == TYPE_COMMAND;
    }

    public boolean isRawKeyEvent() {
        return mEventType == TYPE_RAW_KEY;
    }
    ...
    public boolean isRawPress() {
        return mRawPress;
    }

    public String getBindingSignature() {
        return mBindingSignature;
    }

    public boolean isRawRemapped() {
        return mRawRemapped;
    }
    ...
    public String toString() {
        ...
        sb.append("type=");
        sb.append(mEventType == TYPE_RAW_KEY ? "raw" : "command");
        if (mEventType == TYPE_RAW_KEY) {
            sb.append(", group=");
            sb.append(mRawGroup);
            sb.append(", number=");
            sb.append(mRawNumber);
            sb.append(", press=");
            sb.append(mRawPress);
        } else {
            sb.append(", cmd=");
            ...
            sb.append(", arg=");
            sb.append(mArgument);
            if (mBindingSignature != null) {
                sb.append(", binding=");
                sb.append(mBindingSignature);
            }
            if (mRawRemapped) {
                sb.append(", rawRemapped=true");
            }
        }
        sb.append("}");
        return sb.toString();
    ...
    public void writeToParcel(Parcel out, int flags) {
        ...
        out.writeInt(mRawGroup);
        out.writeInt(mRawNumber);
        out.writeInt(mRawPress ? 1 : 0);
        out.writeString(mBindingSignature);
        out.writeInt(mRawRemapped ? 1 : 0);
    }
    ...
    private BrailleInputEvent(Parcel in) {
        ...
        mRawGroup = in.readInt();
        mRawNumber = in.readInt();
        mRawPress = in.readInt() != 0;
        mBindingSignature = in.readString();
        mRawRemapped = in.readInt() != 0;
    }
}
