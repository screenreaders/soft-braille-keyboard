...
import com.dalton.braillekeyboard.BrailleCommandRemapper;
...
public class DriverThread {
    ...
    private final boolean mUsePollingInput;
    ...
    public DriverThread(Context context, OutputStream outputStream,
        ...
        mUsePollingInput = deviceInfo != null && deviceInfo.isUsb();
        ...
        mHandlerThread = new HandlerThread("DriverTrhead") {
            ...
            protected void onLooperPrepared() {
                ...
                if (success) {
                    mDisplayProperties = mBrlttyWrapper.getDisplayProperties();
                    configureNativeRawRemap();
                    initListener.onInit(mDisplayProperties);
                } else {
    ...
    private void configureNativeRawRemap() {
        if (mDisplayProperties == null || mDeviceInfo == null) {
            mBrlttyWrapper.configureRawRemap(new String[0], new int[0],
                    new int[0], new int[0]);
            return;
        }
        BrailleCommandRemapper.RawRemapConfig config =
                BrailleCommandRemapper.buildRawRemapConfig(mContext,
                        mDeviceInfo.getDeviceAddress(), mDisplayProperties);
        mBrlttyWrapper.configureRawRemap(config.signatures,
                config.targetCommands, config.targetArgumentModes,
                config.targetArgumentValues);
    }
    ...
    private class DriverHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            case MSG_READ:
                ...
                while (true) {
                    BrailleInputEvent inputEvent = mBrlttyWrapper.readInputEvent();
                    if (inputEvent == null) {
                        break;
                    }
                    sawCommand = true;
                    if (!inputEvent.isCommandEvent()) {
                        mInputEventListener.onInputEvent(inputEvent);
                        continue;
                    }
                    int commandCode = inputEvent.getCommand();
                    if (!inputEvent.isRawRemapped()) {
                        commandCode = BrailleCommandRemapper.remapCommand(mContext,
                                mDeviceInfo, mDisplayProperties, commandCode);
                    }
                    long eventTime = inputEvent.getEventTime() != 0
                            ? inputEvent.getEventTime()
                            : SystemClock.uptimeMillis();
                    mInputEventListener.onInputEvent(new BrailleInputEvent(
                            commandCode, inputEvent.getArgument(), eventTime,
                            inputEvent.getEventType(),
                            inputEvent.getRawGroup(),
                            inputEvent.getRawNumber(),
                            inputEvent.isRawPress(),
                            inputEvent.getBindingSignature(),
                            inputEvent.isRawRemapped()));
                }
                if (mUsePollingInput) {
                    removeMessages(MSG_READ);
                    sendEmptyMessageDelayed(MSG_READ, sawCommand ? 60 : 120);
                }
                break;
